...
WITH
  -- Step A: Calculate base features in the first CTE.
  -- This replaces the slow, disk-based temporary table.
  base_features AS (
    SELECT
      t.*,
      ...
      CASE
          WHEN ST_Z(ST_Transform(t.current_position::geometry, 4326)) < 200 THEN 'Loading Zone A'
          WHEN ST_Z(ST_Transform(t.current_position::geometry, 4326)) > 250 THEN 'Dump Zone B'
