...
# This script has been corrected for syntax errors found in the notebook.
...
try:
    ...
    SELECT 
        ...
        CASE 
            WHEN device_id LIKE '%605%' THEN '605_truck'
            WHEN device_id LIKE '%775%' THEN '775G_truck'
            ELSE 'other'
        END as truck_type
    ...
    """
    overview_df = pd.read_sql_query(text(overview_query), engine)
...
try:
    ...
    print("Cell 7: Enum queries successful.")
except Exception as e:
...
try:
    ...
    ORDER BY truck_type;
    """
    quality_by_type_df = pd.read_sql_query(text(quality_by_type_query), engine)
...
try:
    ...
    """
    state_df = pd.read_sql_query(text(state_analysis_query), engine)
...
try:
    ...
    """
    load_weight_df = pd.read_sql_query(text(load_weight_query), engine)
